...
| 行为                     | 策略                                         |
| ------------------------ | -------------------------------------------- |
| 用户删除 md 中的图片语法 | 不立即删文件，保留编辑期间的撤销能力         |
| 手动保存或关闭编辑器     | 扫图片文件与 md 引用，删除未引用图片         |
| 空 `assets/` 目录        | 清理后目录为空则一并删除                     |
| 未知文件或子目录         | 不删除，仅保留目录，避免误删用户数据         |
...
- 2026-08-13：首版实现完成：相对 `assets/`、PNG/JPEG/GIF/WebP、15MB、工具栏选择、粘贴、Live/Preview data URL、缺图占位。
- 2026-08-13：SVG 仍禁用；拖拽本地图片、手动清理未引用图片、手动导入 `http/https` 外链图片已实现。外链默认仍只渲染、不自动下载；真实 Zotero/WebDAV 双端同步仍需按 §5.3 手工探针确认。
